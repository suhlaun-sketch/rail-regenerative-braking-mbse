...
switch code
    ...
    case '7200'
        lines = [lines; string({'persistent pcyl FmechState;'; 'if isempty(pcyl), pcyl = 0.0; FmechState = 0.0; end'; sprintf('dt = %s;',dt);
            ...
            'FregenActual = max(0.0,in_achieved_dynamic_brake_force);'; 'Ftarget = max(Ftotal-FregenActual,0.0);';
            sprintf('if Ftarget >= FmechState, tauF = %.17g; else, tauF = %.17g; end',P.brake.mechanical_force_build_time_s,P.brake.mechanical_force_release_time_s);
            'alphaF = 1.0-exp(-dt/tauF);'; 'FmechState = FmechState + alphaF*(Ftarget-FmechState);';
    ...
    case '8100'
        lines = [lines; string({sprintf('gear = %.17g;',P.driveline.gear_ratio); sprintf('eta = %.17g;',P.driveline.efficiency);
            ...
            'out_achieved_dynamic_brake_force = regenFade*max(-in_motor_torque_actual*gear*eta/rw,0.0);';
